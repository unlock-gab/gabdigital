...
import { Trash2, Search, Eye, Mail, Phone, Calendar, Globe, CheckCircle, Loader2 } from "lucide-react";
import { getProjectRequests, patchProjectRequest, deleteProjectRequest } from "@/lib/api";
...
export default function AdminProjectRequests() {
  ...
  async function loadRequests() {
    ...
    setRequests(rows as ProjectRequest[]);
    setLoading(false);
  }
  ...
  const filtered = requests.filter(
    (r) =>
      ...
      r.requestedService.toLowerCase().includes(search.toLowerCase()) ||
      r.email.toLowerCase().includes(search.toLowerCase()),
  );

  async function markReviewed(id: number) {
    await patchProjectRequest(id, "Reviewed");
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            {newCount > 0 && (
              <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30">{newCount} New</Badge>
            )}
          </div>
          <Button
            size="sm"
            ...
            <tbody className="divide-y divide-slate-800">
              {loading ? (
                ...
                filtered.map((req) => (
                  <tr
                    key={req.id}
                    className={`hover:bg-slate-800/50 transition-colors cursor-pointer ${req.status === "New" ? "bg-slate-900" : "bg-slate-950"}`}
                    onClick={() => setSelected(req)}
                  >
                    <td className="px-4 py-3">
                      <div className={`font-medium ${req.status === "New" ? "text-white" : "text-slate-400"}`}>
                        {req.fullName}
                      </div>
          ...
          {selected && (
            <div className="space-y-4 text-sm">
              ...
              <div className="space-y-2 text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  <Calendar size={14} />
                  <span>Submitted: {selected.date}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div>
                  <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">Business Type</p>
                  <p className="text-white">{selected.businessType}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">Requested Service</p>
                  <p className="text-white">{selected.requestedService}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">Budget</p>
                  <p className="text-orange-400 font-medium">{selected.budget}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">Preferred Start</p>
                  <p className="text-white">{selected.preferredStartDate}</p>
                </div>
              </div>
              <div>
                <p className="text-slate-500 text-xs uppercase tracking-wide mb-1">Project Description</p>
                <p className="text-slate-300 leading-relaxed bg-slate-800 rounded-lg p-3">{selected.description}</p>
              </div>
              {selected.status === "New" && (
                <Button
